...
void checkEdge(int v1, int v2, Grafo* graph)
{
    ...
    if(erro == 0) {
        while (check) { //enquanto check for diferente de NULL, procura no nodo seguinte
            if(check->dest != v2) {
                check = check->next;
            } else {
...
void removeEdge(int v1, int v2, Grafo* grafo)
{
    ...
    //checa se v2 está dentro do escopo do tamanho de grafo
        ...
        while(node) {
            if (node->dest == v2) { //checa se valor de destino de nodo é v2
               //se for, remove ele do nodo
                aux->next = node->next;
                free(node);
                printf("Aresta v1:v2 removida\n");
                break;
            } else if (node->next == NULL) { //senão, ele checa se o proximo ponteiro é nulo
